...
int partition(vector<Bid>& bids, int begin, int end) {
    ...
    while (!done) {
        ...
        while (bids[low].title < pivot) low += 1;

        // keep decrementing high index while bids[pivot] < bids[high]
        while (pivot < bids[high].title) high -= 1;
...
void selectionSort(vector<Bid>& bids) {
    ...
    for (pos; pos < size_t; pos++) {
        
        // set min = pos
        min = pos;

        // loop over remaining elements to the right of position
        for (int next = pos + 1; next < size_t; next++) {
            
            // if this element's title is less than minimum title
            // this element becomes the minimum
            if (bids[next].title < bids[min].title) min = next;
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 4:
            ...
            // FIXME: QuickSort requires two calls to properly sort bids

            /* Secondary quicksort ensures proper sort. Despite following the
               standard algorithm for quicksort, the program does not properly sort
               the bids by title on the first round. */
            quickSort(bids, 0, bids.size() - 1);
